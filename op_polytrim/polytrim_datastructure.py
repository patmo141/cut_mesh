...
class PolyLineKnife(object):
    ...
    def hover(self,context,x,y):
        ...
        ## Update data structures based on the input points and the faces they lay on.
    ...
    def make_cut(self):
        ...
        for m, ind in enumerate(self.face_changes):
            ...
            f0 = self.bme.faces[self.points_data[ind]["face_index"]]  #<<--- Actualy get the BMFace from the cut_object BMEesh
            self.face_chain.add(f0)
            
            f1 = self.bme.faces[self.points_data[n_p1]["face_index"]] #<<--- Actualy get the BMFace from the cut_object BMEesh
            
            ###########################
            ## Define the cutting plane for this segment#
            ############################
            
            no0 = self.points_data[ind]["normal"]  #direction the user was looking when defining the cut
            no1 = self.points_data[n_p1]["normal"]  #direction the user was looking when defining the cut
    ...
    def draw(self,context): 
        ...
        if len(self.bad_segments):
            for ind in self.bad_segments:
                ...
                common_drawing.draw_polyline_from_3dpoints(context, [self.points_data[ind]["world_location"], self.points_data[ind_p1]["world_location"]], (1,.1,.1,1), 4, 'GL_LINE')
